 ...
 */
export function calcDobanda(principal, rate) {
 ...
 */
export function calcSoldNou(principal, rate) {
 ...
 */
export function convertValuta(suma, curs, directie) {
    const dec = new Decimal(suma);
    const rezultat = directie === "RON_EUR"
        ? dec.div(curs)
        : dec.times(curs);
