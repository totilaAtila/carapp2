 ...
 */
export function validateDb(buffer: ArrayBuffer): boolean {
 ...
 */
export async function detectDualCurrency(
  log?: ProgressFn
): Promise<{ active: boolean; suffix: string | null }> {
  try {
    ...
    if (jsonResp.ok) {
      ...
      const active =
        data?.converted || data?.active || data?.eur || data?.use_eur || data?.mode === "EUR";
      if (active) {
...
export async function loadDb(
  ...
  for (const n of possibleNames) {
    try {
      ...
      log?.(`⚠️ Eroare la încărcarea ${n}: ${(err as Error).message}`);
      continue;
    }
  }
 ...
 */
export async function loadAllDbs(
  sql: SqlJsStatic,
  suffixHint: string | null,
  log?: ProgressFn
): Promise<DBSet> {
