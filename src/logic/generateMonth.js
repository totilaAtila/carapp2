...
const log = (p, m) => { if (p && m)
    p(m); };
const D0 = (v) => new Decimal(v || 0);
const Q2 = (v) => new Decimal(v || 0).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
const NEAR_ZERO = new Decimal("0.005");
function getLastPeriod(depcred) {
...
function getLichidatiSet(lichidati) {
    const s = new Set();
    if (!lichidati)
        return s;
...
function getMembri(membrii) {
    const info = membrii.exec("PRAGMA table_info(membrii);");
    if (!info.length)
        throw new Error("Tabelul 'membrii' lipsește.");
...
function getSourceBalances(depcred, nr_fisa, year, month) {
    const r = depcred.exec("SELECT impr_sold, dep_sold FROM depcred WHERE nr_fisa=? AND anul=? AND luna=? LIMIT 1;", [nr_fisa, year, month]);
...
function getInheritedLoanRate(depcred, nr_fisa, year, month) {
    const r = depcred.exec("SELECT impr_deb, impr_cred FROM depcred WHERE nr_fisa=? AND anul=? AND luna=? LIMIT 1;", [nr_fisa, year, month]);
    if (!r.length || !r[0].values.length)
        return Q2(0);
...
function getDividend(activi, nr_fisa) {
    if (!activi)
        return Q2(0);
...
function getLoanStart(depcred, nr_fisa, yM1) {
    const r = depcred.exec("SELECT MAX(anul*100+luna) FROM depcred WHERE nr_fisa=? AND impr_deb>0 AND (anul*100+luna)<=?;", [nr_fisa, yM1]);
    if (!r.length || !r[0].values.length || r[0].values[0][0] == null)
        return null;
    return Number(r[0].values[0][0]);
}
function calcExtinctionInterest(depcred, nr_fisa, sourceYear, sourceMonth) {
    const yM1 = sourceYear * 100 + sourceMonth;
    const start = getLoanStart(depcred, nr_fisa, yM1);
    if (start == null)
        return Q2(0);
    const r = depcred.exec("SELECT SUM(impr_sold) FROM depcred WHERE nr_fisa=? AND (anul*100+luna BETWEEN ? AND ?) AND impr_sold>0;", [nr_fisa, start, yM1]);
    const sumPos = Q2(r[0].values[0][0]);
    if (sumPos.lte(0))
        return Q2(0);
    return Q2(sumPos.mul("0.004"));
}
function insertDepcredRow(depcred, nr_fisa, y, m, dep_deb, dep_cred, dep_sold, impr_deb, impr_cred, impr_sold, dobanda) {
    depcred.run("INSERT INTO depcred (nr_fisa, luna, anul, dobanda, impr_deb, impr_cred, impr_sold, dep_deb, dep_cred, dep_sold, prima) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1);", [
        nr_fisa, m, y,
        Number(dobanda), Number(impr_deb), Number(impr_cred), Number(impr_sold),
        Number(dep_deb), Number(dep_cred), Number(dep_sold)
    ]);
}
function ensureTargetNotExists(depcred, y, m) {
...
function zeroizeIfNearZero(v) {
    return v.abs().lte(NEAR_ZERO) ? Q2(0) : v;
}
export function generateMonth(opts) {
    ...
    for (const m of membri) {
        ...
        if (lich.has(nr)) {
            log(onProgress, `⭕ ${nr} (${nume}) este lichidat.`);
            continue;
        }
        const src = getSourceBalances(depcredDb, nr, sourceYear, sourceMonth);
        if (!src) {
            log(onProgress, `⚠️ Lipsă rând sursă pentru fișa ${nr} în ${String(sourceMonth).padStart(2, "0")}-${sourceYear}`);
            continue;
        }
        ...
        insertDepcredRow(depcredDb, nr, targetYear, targetMonth, dep_deb_nou, dep_cred_nou, dep_sold_nou, impr_deb_nou, impr_cred_nou, impr_sold_nou, dobanda);
        cnt++;
        S_dep_sold_nou = S_dep_sold_nou.add(dep_sold_nou);
        S_impr_sold_nou = S_impr_sold_nou.add(impr_sold_nou);
        if (cnt % 50 === 0)
            log(onProgress, `➕ Procesate ${cnt} fișe...`);
    }
...
export function deleteMonth(db, month, year) {
