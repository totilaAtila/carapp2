...
export default function StergeMembru({ databases }) {
  ...
  // Verificare completă a ștergerii
  const verificaStergereCompleta = async (nrFisa) => {
    try {
      const databasesToVerify = [
        { db: 'membrii', table: 'MEMBRII', key: 'NR_FISA' },
        { db: 'depcred', table: 'DEPCRED', key: 'NR_FISA' },
        { db: 'activi', table: 'ACTIVI', key: 'NR_FISA' },
        { db: 'inactivi', table: 'inactivi', key: 'nr_fisa' },
  ...
  // Aplică efecte de umbră premium 3D elementelor
  const applyPremiumShadow = (element) => {
    if (element) {
      element.style.boxShadow = '0 10px 30px rgba(0, 0, 0, 0.15), 0 4px 15px rgba(0, 0, 0, 0.1)';
      element.style.transition = 'transform 0.3s ease, box-shadow 0.3s ease';
      element.onmouseover = () => {
        element.style.transform = 'translateY(-2px)';
        element.style.boxShadow = '0 12px 35px rgba(0, 0, 0, 0.2), 0 5px 20px rgba(0, 0, 0, 0.15)';
      };
      element.onmouseout = () => {
  ...
  useEffect(() => {
    // Aplică efecte de umbră premium la secțiunile importante
    const sectionElements = document.querySelectorAll('.premium-section');
    sectionElements.forEach(applyPremiumShadow);
    
    const headerElements = document.querySelectorAll('.premium-header');
    headerElements.forEach(applyPremiumShadow);
  }, [membruData, istoric]);
        ...
        _jsxs(Card, { className: "hidden lg:block border-0 shadow-2xl mb-8 premium-section", children: [
          _jsx(CardHeader, { className: "bg-gradient-to-r from-slate-100 to-slate-300 border-b-3 border-slate-400 pb-4 premium-header", children: 
            _jsxs(CardTitle, { className: "text-xl font-extrabold text-slate-800 flex items-center gap-4", children: [
              ...
              _jsx("span", { className: "text-slate-600 ml-2" }, ` (Fișa: ${membruData.nr_fisa})`),
              _jsx("div", { className: "w-4 h-4 bg-blue-600 rounded-full drop-shadow-md" })
            ]})
          }),
        ...
        _jsx(CardContent, { children: 
          logs.length > 0 ? (
            _jsx("div", { className: "bg-slate-900 text-green-400 p-5 rounded-xl font-mono text-sm max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-green-500 scrollbar-track-slate-800", children: 
              logs.map((log, idx) => (
                _jsx("div", { className: `border-b border-slate-800 py-2 ${idx === logs.length - 1 ? 'border-b-0' : ''} ${log.includes('✅') ? 'text-green-300' : log.includes('⚠️') || log.includes('❌') ? 'text-red-300' : 'text-blue-300'} hover:bg-slate-800/50 transition-colors`, children: log }, idx)
              ))
            })
          ) : (
