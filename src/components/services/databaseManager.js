...
function validateDatabaseStructure(db, name) {
    ...
    try {
        ...
        const tables = ((_a = res[0]) === null || _a === void 0 ? void 0 : _a.values.flat()) || [];
...
export function loadDatabasesFromFilesystem() {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c, _d, _e;
        if (!("showDirectoryPicker" in window)) {
            console.warn("⚠️ File System Access API indisponibil — se folosește fallback upload");
            const result = yield loadDatabasesFromUpload();
        ...
        try {
            ...
            const membrii = yield loadDatabaseFile(sql, dirHandle, "MEMBRII.db", true);
            const depcred = yield loadDatabaseFile(sql, dirHandle, "DEPCRED.db", true);
            const activi = yield loadDatabaseFile(sql, dirHandle, "activi.db", true);
            const inactivi = yield loadDatabaseFile(sql, dirHandle, "INACTIVI.db", true);
            const lichidati = yield loadDatabaseFile(sql, dirHandle, "LICHIDATI.db", true);
            // ========== ÎNCĂRCARE BAZE EUR (Opționale - dar trebuie SET COMPLET) ==========
            console.log("📂 Încărcare baze EUR (opționale)...");
            const membriieur = yield loadDatabaseFile(sql, dirHandle, "MEMBRIIEUR.db", true);
            const depcredeur = yield loadDatabaseFile(sql, dirHandle, "DEPCREDEUR.db", true);
            const activieur = yield loadDatabaseFile(sql, dirHandle, "activiEUR.db", true);
            const inactivieur = yield loadDatabaseFile(sql, dirHandle, "INACTIVIEUR.db", true);
            const lichidatieur = yield loadDatabaseFile(sql, dirHandle, "LICHIDATIEUR.db", true);
            // ========== ÎNCĂRCARE CHITANTE.db (OBLIGATORIU - comun) ==========
            console.log("📂 Încărcare CHITANTE.db (obligatoriu)...");
            const chitante = yield loadDatabaseFile(sql, dirHandle, "CHITANTE.db", false);
            ...
            yield clearAllPersistedDatabases();
            ...
            console.error("  - constructor:", (_a = err.constructor) === null || _a === void 0 ? void 0 : _a.name);
            ...
            if ((_b = err.message) === null || _b === void 0 ? void 0 : _b.includes('Permisiuni refuzate')) {
                throw err; // Re-throw cu mesajul original detaliat
            }
            // Erori de validare (baze lipsă, corupte)
            if (((_c = err.message) === null || _c === void 0 ? void 0 : _c.includes('lipsește')) ||
                ((_d = err.message) === null || _d === void 0 ? void 0 : _d.includes('coruptă')) ||
                ((_e = err.message) === null || _e === void 0 ? void 0 : _e.includes('nu conține'))) {
...
function loadDatabaseFile(sql_1, dirHandle_1, fileName_1) {
    return __awaiter(this, arguments, void 0, function* (sql, dirHandle, fileName, optional = false) {
        var _a, e_1, _b, _c;
        ...
        try {
            ...
            try {
                ...
                for (var _d = true, _e = __asyncValues(dirHandle.values()), _f; _f = yield _e.next(), _a = _f.done, !_a; _d = true) {
                    _c = _f.value;
                    _d = false;
                    const entry = _c;
                    if (entry.kind === "file") {
...
export function saveDatabaseToFilesystem(dirHandle, fileName, db) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (!db)
            ...
            if ("showSaveFilePicker" in window && (dirHandle === null || dirHandle === void 0 ? void 0 : dirHandle.createWritable)) {
...
export function persistDatabases(databases) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (!databases)
                return;
            if (databases.source === "filesystem" && databases.folderHandle) {
                ...
                yield saveDatabaseToFilesystem(databases.folderHandle, "CHITANTE.db", databases.chitante);
                ...
                if (databases.availableCurrencies.includes("RON")) {
                    ...
                    if (databases.membrii)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "MEMBRII.db", databases.membrii);
                    if (databases.depcred)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "DEPCRED.db", databases.depcred);
                    if (databases.activi)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "activi.db", databases.activi);
                    if (databases.inactivi)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "INACTIVI.db", databases.inactivi);
                    if (databases.lichidati)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "LICHIDATI.db", databases.lichidati);
                }
                ...
                if (databases.availableCurrencies.includes("EUR")) {
                    ...
                    if (databases.membriieur)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "MEMBRIIEUR.db", databases.membriieur);
                    if (databases.depcredeur)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "DEPCREDEUR.db", databases.depcredeur);
                    if (databases.activieur)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "activiEUR.db", databases.activieur);
                    if (databases.inactivieur)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "INACTIVIEUR.db", databases.inactivieur);
                    if (databases.lichidatieur)
                        yield saveDatabaseToFilesystem(databases.folderHandle, "LICHIDATIEUR.db", databases.lichidatieur);
                }
                ...
                if (isIOS) {
                    ...
                    yield downloadDatabaseWithDelay("CHITANTE.db", databases.chitante, 800);
                    // ========== DOWNLOAD BAZE RON (dacă există) ==========
                    if (databases.availableCurrencies.includes("RON")) {
                        console.log("📥 Download baze RON...");
                        if (databases.membrii)
                            yield downloadDatabaseWithDelay("MEMBRII.db", databases.membrii, 800);
                        if (databases.depcred)
                            yield downloadDatabaseWithDelay("DEPCRED.db", databases.depcred, 800);
                        if (databases.activi)
                            yield downloadDatabaseWithDelay("activi.db", databases.activi, 800);
                        if (databases.inactivi)
                            yield downloadDatabaseWithDelay("INACTIVI.db", databases.inactivi, 800);
                        if (databases.lichidati)
                            yield downloadDatabaseWithDelay("LICHIDATI.db", databases.lichidati, 800);
                    }
                    // ========== DOWNLOAD BAZE EUR (dacă există) ==========
                    if (databases.availableCurrencies.includes("EUR")) {
                        console.log("📥 Download baze EUR...");
                        if (databases.membriieur)
                            yield downloadDatabaseWithDelay("MEMBRIIEUR.db", databases.membriieur, 800);
                        if (databases.depcredeur)
                            yield downloadDatabaseWithDelay("DEPCREDEUR.db", databases.depcredeur, 800);
                        if (databases.activieur)
                            yield downloadDatabaseWithDelay("activiEUR.db", databases.activieur, 800);
                        if (databases.inactivieur)
                            yield downloadDatabaseWithDelay("INACTIVIEUR.db", databases.inactivieur, 800);
                        if (databases.lichidatieur)
                            yield downloadDatabaseWithDelay("LICHIDATIEUR.db", databases.lichidatieur, 800);
                    }
                }
                else {
...
export function downloadDatabaseWithDelay(fileName, db, delayMs) {
    return __awaiter(this, void 0, void 0, function* () {
